...
Ticket_Query = os.environ['TICKET_BLOB_QUERY']
...
blob_client = container_client.get_blob_client(Ticket_Query)
# Download the query file as text
Ticket_query_text = blob_client.download_blob().readall().decode("utf-8")
...
def log_error_and_abort(message):
    ...
    try:
        ...
        with cnxn1.cursor() as cursor_error:
            cursor_error.execute("INSERT INTO [dbo].[DMS_Ticket_Creation_Logs] (Status, Total_Rows_Available, Timestamp) VALUES (?, ?, ?)",
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    with cnxn.cursor() as upload_cursor:
        ...
        try:
            ...
            upload_cursor.execute(Ticket_query_text)
            ...
            new_row = {'Status': 'Data Loaded successfully', 'Total_Rows_Available': len(DMS_Ticket_Query_df),'Timestamp': today}
        ...
        with cnxn1.cursor() as cursor_insert:
            cursor_insert.execute("INSERT INTO [dbo].[DMS_Ticket_Creation_Logs] (Status, Total_Rows_Available, Timestamp) VALUES (?, ?, ?)",
                      new_row['Status'], new_row['Total_Rows_Available'],new_row['Timestamp'])
            cnxn1.commit()
        cursor_insert.close()
